...
button {
  ...
  <div id="测试excalidraw.md">
      <iframe id="excalidrawIframe" style="display:none; width:100%; height:100%; position:fixed; top:0; left:0; border:none;"></iframe>
      <!-- 添加全屏按钮 -->
      <button id="fullscreenBtn" style="position: fixed; z-index: 10; left: 20px; top: 20px;">全屏显示Excalidraw</button>
  </div>
  <script>
  (function(){
    const InitialData={
      ...
      "elements":[
        ...
        // 将Excalidraw内容字符串转换为blob URL
        var blob = new Blob([initialData], {type: 'text/html'});
        var url = URL.createObjectURL(blob);
        document.getElementById('fullscreenBtn').addEventListener('click', function() {
          var iframe = document.getElementById('excalidrawIframe');
          // 确保在这里定义了excalidrawContent变量，且包含Excalidraw的内容
          var excalidrawContent = '这里应该是你的Excalidraw内容';
          var blob = new Blob([excalidrawContent], {type: 'text/html'});
          var url = URL.createObjectURL(blob);
          iframe.src = url;
